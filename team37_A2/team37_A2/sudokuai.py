...
class SudokuAI(competitive_sudoku.sudokuai.SudokuAI):
    ...
    def compute_best_move(self, game_state: GameState) -> None:
        ...
        self.minimax = MiniMax(manager, self.logger)

        # Iterative deepening variant of minimax (keeps going until time limit reached)
        while True:
            # Minimax computes the best move at the current depth and increments the depth by 1
            _, best_move_score = self.minimax.iterative_deepening()
            # Log the best move - Can be called multiple times, will overwrite the previous best move
            self.propose_move(best_move_score.move)

    def get_depth_stats(self):
        return self.minimax.get_depth_stats()
...
class GameStateManager:
    ...
    Basically it can tell the current state, which consists of 2 variables: initial_state, moves_performed
    This computes the 'current_state', by applying the 'moves_performed' on the 'initial_state'. We compute the 'current_state' before computing the legal_moves and before evaluating the state.
    Then after evaluating the state, we undo the 'moves_performed' such that we have a clean initial state. By having this structure we don't have to do any deepcopies, which significantly speeds up the runtime.
    """
    ...
    def __init__(
        ...
        remaining_earnable_points=None,
        points_earned=0,
    ):
        self.initial_gamestate = initial_gamestate
        self.maximizing_player = initial_gamestate.current_player
        self.moves = moves_performed
        ...
        self.scores = initial_gamestate.scores.copy() if scores == None else scores
        self.steps = steps
        self.REMAINING_EARNABLE_POINTS = (
            remaining_earnable_points
            if remaining_earnable_points
            else self._compute_remaining_grid_score()
        )
        self.additional_points_earned = (
            points_earned  # How many points have been earned in this evaluation depth
        )

    # Do moves to the current game state --> Return the new state
    def do_move(self, move: MoveScore):
        # Create a new list of moves by appending the current move
        new_moves_performed = self.moves.copy() + [move]

        new_points_earned = self.additional_points_earned + move.score
        ...
        score_player = self.scores[self.maximizing_player - 1]
        score_opp = self.scores[2 - self.maximizing_player]

        # Determine who should get the move score
        current_player = (
            self.initial_gamestate.current_player
            if self.steps % 2 == 0
            else 3 - self.initial_gamestate.current_player
        )

        if current_player == self.maximizing_player:
            score_player += move.score
        else:
            score_opp += move.score

        winner_determined = new_manager._detemine_if_there_if_is_a_winner_already(
            new_manager, score_player, score_opp
        )
    ...
    def evaluate_state(self):
        sim_state = self._apply_moves()

        # Evaluate the state
        # calculate possible mves of player 1 (curr)
        score_player = sim_state.scores[self.maximizing_player - 1]

        # calculate possible moves of player 2 (opp)
        score_opponent = sim_state.scores[2 - self.maximizing_player]

        # Get the legal moves for the current player and the opponent
        original_player = sim_state.current_player
        sim_state.current_player = self.maximizing_player
        legal_moves_player = sim_state.player_squares()
        sim_state.current_player = 3 - self.maximizing_player
        legal_moves_opponent = sim_state.player_squares()
        sim_state.current_player = original_player

        # 1 if guaranteed winner. 0 if undecable yet. -1 if lost
        winner_determined = self._detemine_if_there_if_is_a_winner_already(
            sim_state,
            score_player,
            score_opponent,
            legal_moves_player,
        ...
        # Undo everything so we have a clean state
        self._undo_moves()
        ...
        score_heuristic = score_player - score_opponent
        move_difference = len(legal_moves_player) - len(legal_moves_opponent)

        # Insentive to reduce opponent moves - The less move the opponent has, the more unlikely it is that the opponent do a good move.
        if len(legal_moves_opponent) != 0:
            move_difference /= len(legal_moves_opponent)

        return self._heuristic(score_heuristic, move_difference, winner_determined)
    ...
    def _detemine_if_there_if_is_a_winner_already(
        ...
        points_remaining = (
            self.REMAINING_EARNABLE_POINTS - self.additional_points_earned
        )
    ...
    def _get_legal_moves(self, curr_state: GameState) -> list[MoveScore]:
        ...
        taboo_moves = curr_state.taboo_moves
    ...
    def get_legal_value_for_move(self, best_move: MoveScore):
        ...
        cand_val -= set(
            curr_state.board.get((row, i)) for i in range(dimensions)
        )  # Row
        cand_val -= set(
            curr_state.board.get((i, column)) for i in range(dimensions)
        )  # Column
    ...
    def _precompute_constraints(self):
        """
        Precomputes numbers present in each row, column, and block for faster validation.

        Returns a dict: dictionary containing number for rows, columns, and blocks. Telling the items in the row
        """
    ...
    def _apply_moves(self, for_legal_move_generation=False) -> GameState:
        ...
        Args:
            for_legal_move_generation (bool): Whether the moves are applied for legal move generation.
        Returns:
    ...
    def _heuristic(
        ...
        score = (
            points_heuristic * point_weight
            + move_difference * move_weight
...
class MiniMax:
    ...
    def iterative_deepening(self) -> MoveScore:
        ...
        best_score, best_move = self.minimax_alpha_beta(
            self.state_manager, self.computed_depth, True
        )
        ...
        # Update depth stats
        self.min_depth = min(self.min_depth, self.computed_depth)
        self.max_depth = max(self.max_depth, self.computed_depth)
        self.total_depth += self.computed_depth
    ...
    def get_depth_stats(self):
        avg_depth = self.total_depth / self.depth_count if self.depth_count > 0 else 0
        return self.min_depth, self.max_depth, avg_depth
    ...
    def minimax_alpha_beta(
        ...
        if is_maximizing:
            ...
            for move in moves:
                ...
                new_score, _ = self.minimax_alpha_beta(
    ...
    def _log_results(self, best_move: MoveScore, best_score: int):
        ...
        self.logger.info(
            f"Best move at depth {self.computed_depth}: {best_move} - max state score: {best_score}"
        )
        self.logger.info("~" * 50)
